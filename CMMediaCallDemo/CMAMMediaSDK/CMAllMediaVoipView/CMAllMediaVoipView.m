...
//全屏时状态和计时tilte间隔
static CGFloat stateChatDistance = 6;
//挂断按钮Y
...
//语音通话单例 Agora
@property (nonatomic, strong) CMAllMediaRTCChatManager *agoraManager;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        isLoudMode = NO;
        isShowKeyboard = NO;
        self.agoraManager = [CMAllMediaRTCChatManager sharedInstance];
    }
    return self;
}
...
- (void)callVoipPhoneWithWithNumber:(NSString *)phoneNumber {
    if ([CMAllMediaRTCChatManager sharedInstance].isVoipNormal && ![CMAllMediaRTCChatManager sharedInstance].isCallingFlag) {
        ...
        [weakSelf.agoraManager callUserWithUserNumber:phoneNumber isVideoCall:NO repairOrderNumber:@"" andSetupLargerPreview:nil andShrinkView:nil];
        weakSelf.agoraManager.voipBlock = ^(CMAMRTCState state) {
            [weakSelf handleAgoraStateWithVoipState:state];
        };
        [weakSelf showVoipView];
...
- (void)dismissVoipView {
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [self removeAllSubviewsFromSuperView:self.showView];
...
- (void)hangupVoipPhone {
    [_agoraManager hangupAgoraCallConnect];
}

#pragma mark 根据Agora状态的处理
- (void)handleAgoraStateWithVoipState:(NSInteger)voipState {
    switch (voipState) {
        case CMAMRTCStateAudioCalling:
            ...
            break;
        case CMAMRTCStateAudioIncoming:
            break;
        ...
        case CMAMRTCStateConnecting:
            break;
        case CMAMRTCStateNormalConfirm:
        {
        ...
        {
            [self destoryWaitTimer];
            if (self.agoraVoipBlock) {                
                self.agoraVoipBlock(CMAMRTCStateDisconnect, contenctT?_chatStateLabel.text:nil);
            }
        ...
        case CMAMRTCStateNoResponse:
        {
            [self destoryWaitTimer];
            if (self.agoraVoipBlock) {
                self.agoraVoipBlock(CMAMRTCStateNoResponse, nil);
            }
...
- (void)handleSilenceVolume:(UIButton *)sender {
    if (contenctT > 0) {    
        ...
        [_agoraManager changeIsSilenceWithFlag:isSilence];
    }
}
...
- (void)handleChangeVoiceMethod:(UIButton *)sender {
    if (contenctT > 0) {
        ...
        [_agoraManager changeVoiceWithLoudSpeaker:isLoudMode];
    }
}
...
- (void)handelShowNumberKeyboard:(UIButton *)sender {
    ...
    _keyboardView.numberBlock = ^(NSString *numberStr) {
        [weakSelf.agoraManager sendDTMFDataString:numberStr];
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        _serviceHeaderView.frame = CGRectMake(serverRect.origin.x+serverRect.size.width/2, _shrinkBtn.frame.origin.y+_shrinkBtn.frame.size.height, 0, 0);
        
        _chatStateLabel.frame = CGRectMake(chatStateRect.origin.x, _shrinkBtn.frame.origin.y+_shrinkBtn.frame.size.height, chatStateRect.size.width, chatStateRect.size.height);
        _stateLabel.frame = CGRectMake(stateRect.origin.x, _chatStateLabel.frame.origin.y+_chatStateLabel.frame.size.height+stateChatDistance*kCMScaleHeight, stateRect.size.width, stateRect.size.height);
    } completion:^(BOOL finished) {
...
- (void)handleHiddenNumberKeyboard:(UIButton *)sender {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        _chatStateLabel.frame = CGRectMake(chatStateRect.origin.x, _shrinkBtn.frame.origin.y+_shrinkBtn.frame.size.height+chatLabelY*kCMScaleHeight, chatStateRect.size.width, chatStateRect.size.height);
        _stateLabel.frame = CGRectMake(stateRect.origin.x, _chatStateLabel.frame.origin.y+_chatStateLabel.frame.size.height+stateChatDistance*kCMScaleHeight, stateRect.size.width, stateRect.size.height);
        
        _serviceHeaderView.hidden = NO;
        _serviceHeaderView.frame = CGRectMake((kCMScreenWidth-headerWH*kCMScaleWidth)/2, _shrinkBtn.frame.origin.y+_shrinkBtn.frame.size.height, headerWH*kCMScaleWidth, headerWH*kCMScaleWidth);
    }];
}
...
- (void)handlePanVoipView:(UIPanGestureRecognizer *)sender {
    ...
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        if (minSpace == top) {//上
            ...
            newCenter = CGPointMake(endX , edgeDistance*(kCMISiPhoneX?4:2) + kSmallH + 2);
        } else if(minSpace == bottom) {//下
            endX = endX - kSmallW < edgeDistance * 2 ? kSmallW + edgeDistance : endX;
            endX = endX + kSmallW > superWidth - edgeDistance * 2 ? superWidth - kSmallW - edgeDistance : endX;
            newCenter = CGPointMake(endX , superHeight - kSmallH - edgeDistance*(kCMISiPhoneX?3:1) + 2);
...
- (void)updateStateLabelWithStr:(NSString *)str {
    ...
    if (isSmall) {
        ...
        _chatStateLabel.frame = CGRectMake((_showView.frame.size.width-chatSize.width)/2, 35*kCMScaleWidth, chatSize.width, 20);
    } else {
        ...
        _chatStateLabel.frame = CGRectMake((kCMScreenWidth-chatSize.width)/2, isShowKeyboard?(_shrinkBtn.frame.origin.y+_shrinkBtn.frame.size.height):(_serviceHeaderView.frame.origin.y+_serviceHeaderView.frame.size.height+13*kCMScaleWidth), chatSize.width, 25);
        
        _stateLabel.text = stateStr;
        _stateLabel.frame = CGRectMake((kCMScreenWidth-stateSize.width)/2, _chatStateLabel.frame.origin.y+_chatStateLabel.frame.size.height+stateChatDistance*kCMScaleWidth, stateSize.width, 20);
    }
}
...
- (UIImageView *)serviceHeaderView {
    if (!_serviceHeaderView) {
        _serviceHeaderView = [[UIImageView alloc] initWithFrame:CGRectMake((kCMScreenWidth-headerWH*kCMScaleWidth)/2, _shrinkBtn.frame.origin.y+_shrinkBtn.frame.size.height, headerWH*kCMScaleWidth, headerWH*kCMScaleWidth)];
        [_serviceHeaderView setImage:kCM_loadBundleImage(@"serverheadbig")];
    }
...
- (UILabel *)chatStateLabel {
    if (!_chatStateLabel) {
        _chatStateLabel = [[UILabel alloc] initWithFrame:CGRectZero];
...
- (UIButton *)hiddenKeyboardBtn {
    if (!_hiddenKeyboardBtn) {
        ...
        _hiddenKeyboardBtn.frame = CGRectMake(_shrinkBtn.frame.origin.x, _hangupButton.frame.origin.y, _keyboardView.frame.size.width / 3, buttonWidth*kCMScaleHeight);
        _hiddenKeyboardBtn.hidden = YES;
...
- (UIButton *)deleteBtn {
    if (!_deleteBtn) {
        ...
        _deleteBtn.frame = CGRectMake(_showView.frame.size.width-((_showView.frame.size.width-2*_shrinkBtn.frame.origin.x) / 3)-_shrinkBtn.frame.origin.x, _hangupButton.frame.origin.y, _keyboardView.frame.size.width / 3, buttonWidth*kCMScaleHeight);
        _deleteBtn.hidden = YES;
...
- (CMAllMediaNumberKeyboardView *)keyboardView {
    if (!_keyboardView) {
        _keyboardView = [[CMAllMediaNumberKeyboardView alloc] initWithFrame:CGRectMake(_shrinkBtn.frame.origin.x, numberKeyBoardY*kCMScaleWidth, _showView.frame.size.width-2*_shrinkBtn.frame.origin.x, _hangupButton.frame.origin.y-(numberKeyBoardY+20)*kCMScaleWidth)];
        _keyboardView.hidden = YES;
    }
...
- (CALayer *)bottomLayer {
    if (!_bottomLayer) {
        ...
        _bottomLayer.frame = CGRectMake(0, self.contentView.frame.size.height - 1, self.contentView.frame.size.width, 1);
...
- (CALayer *)rightLayer {
    if (!_rightLayer) {
        ...
        _rightLayer.frame = CGRectMake(self.contentView.frame.size.width - 1, 0, 1, self.contentView.frame.size.height);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    return CGSizeMake((self.frame.size.width - 2) /3, self.frame.size.height/4);
}
